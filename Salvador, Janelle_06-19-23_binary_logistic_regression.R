...
#feature importance
library(caret)
caret::varImp(model1) #have an idea w/c predictor to remove
...
nrow(train4)
#go back to step 3 and use train4
...
predict <- predict(model2, new, type="response")
predict

#calculate the probability for each individial in test dataset
predicted <- predict(model2, test, type="response")
#predicted

#step 7: model diagnostics
#calculate probability of default for each individual in test dataset by setting the cutoff value by >0.5
predicted_classes1 <- ifelse(predicted>0.5, "Yes", "No")
table(predicted_classes1)


library(MKclass)
#find the optimal cutoff probability to use maximize accuracy
optimal <- optCutoff(predicted, truth=test$default, namePos="Yes")
optimal

#calculate probability of default for each individual in test dataset by setting the cutoff value using optimal value
predicted_classes2 <- ifelse(predicted>optimal, "Yes", "No")
table(predicted_classes2)
